...
- **Dollar-quote with `$$`, not a named tag** like `$spec$`. Cortex Code's SQL
  execution path rejects named dollar-quote tags (it errors with
  `unexpected '$spec'`). The spec JSON never contains `$$`, so plain `$$` is safe.
- **`models.orchestration` must be `"auto"`.** Agent orchestration has a narrower,
  account-specific allowed-models list than Cortex COMPLETE, so a pinned model
  can fail with *"not an allowed model for Agent"*. Pin it later in Snowsight
  under **Configuration → Model** if wanted; `claude-sonnet-4-5` is a good pick
  where offered (mind the id — `claude-sonnet-4-5`, not `claude-4-sonnet`).
- **To change the agent, re-run the whole `CREATE OR REPLACE AGENT ... FROM
  SPECIFICATION` statement.** Do not attempt a workspace-file edit/redeploy path;
  this agent is created from SQL and is not tracked in a workspace, so that fails
  with *"Could not resolve workspace file ... cortex-project.yaml"*.
- **`ALTER ACCOUNT SET CORTEX_ENABLED_CROSS_REGION = 'ANY_REGION'`** must already
  have run (`00_bootstrap.sql`, BLOCK 1) or the agent degrades or fails.
- After creating it, send the attendee to **Snowsight → AI & ML → Agents →
  Cascade Plant Analyst** and the chat panel on the detail page. Not Cortex Code.
  They do **not** need to Publish — the agent already exists from SQL.
...
1. *"What is first-pass yield by line right now?"* — Gold table only.
2. *"Which defect is driving scrap on PAINT?"* — defect counts, excluding `NONE`.
3. *"Why did PAINT yield drop?"* — **the payoff.** Requires reading across both
   sources: the CDC scan feed for yield, and the streamed sensor telemetry for
   booth humidity, plus noticing that humidity rose *before* the defects did. An
   agent on the CDC feed alone can only report what happened, never why.
