...
| Dashboard: `Gold lag` reads minutes, not seconds | The warehouse is the constraint, or a refresh is failing | Expect 30–90 s against `TARGET_LAG = '1 minute'`. Read `refresh_mode` and `refresh_mode_reason`; a table on `FULL` refresh lags far more than one on `INCREMENTAL`. |
